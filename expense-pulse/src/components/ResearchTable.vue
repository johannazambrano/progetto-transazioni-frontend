...
<template>
  <div class="card-wrapper">
    <header class="card-header">
      <div class="p-4 sm:p-6 border-b border-gray-100">
        <div class="flex justify-between items-center">
          <h3 class="text-sm sm:text-base font-bold text-gray-800 uppercase tracking-wider">
            Ricerca Avanzata
          </h3>
    ...
    <div class="card-content p-4 sm:p-6" :class="{ 'animate-shake border-red-200': isShakingSearch }">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="space-y-1">
          <label class="text-xs font-semibold text-gray-500 ml-1">Categoria</label>
          <select v-model="filters.category" @change="applyFilters"
            class="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none">
            <option value="">Tutte le categorie</option>
            <option v-for="cat in categoryStore.categories" :key="cat.id" :value="cat.descrizione">
              {{ cat.descrizione }}
            </option>
          </select>
        </div>
        ...
        <div class="space-y-1 relative">
          ...
          <p v-if="filters.startDate && !filters.endDate" class="text-[10px] text-red-500 mt-0.5 absolute left-1">
        ...
        <div class="space-y-1 relative">
          ...
          <p v-if="filters.endDate && !filters.startDate" class="text-[10px] text-red-500 mt-0.5 absolute left-1">
